...
@Component({
  ...
  templateUrl: './associate-list.page.html',
  styleUrls: ['./associate-list.page.scss'],
})
// End here


export class AssociateListPage implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    // ------ init function call start -------
      commonFunction(){
        ...
        this.headerUrlapi = 'associate/header';
    
        // table list data url name
        this.listing_url = 'associate/return_index';
    ...
    onDownload(_identifier, _items, _item){
      ...
      if(_identifier == 'multiple'){
        this.itemsSubscribe = this.authService.globalparamsData.subscribe(res => {
          this.export_url = this.main_url+'/associate/download_csv?id='+items_id_array.join()+'&token='+res.token;
        });

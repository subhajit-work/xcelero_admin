...
/* tslint:disable */ 
@Component({
  selector: 'app-adminmenu-list',
  templateUrl: './adminmenu-list.page.html',
  styleUrls: ['./adminmenu-list.page.scss'],
})
// End here


export class AdminmenuListPage implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    // ------ init function call start -------
      commonFunction(){
        ...
        this.headerUrlapi = 'adminmenu/header';
        
        if(this.parms_action_id != null){
          // console.log('adminmenu child');
          this.listing_url = 'adminmenu/return_index/'+this.parms_action_id;
          this.enable_listing_url = 'adminmenu/return_index';
          this.hasChildList = true;
        }else{
          // console.log('adminmenu');
          // table list data url name
          this.listing_url = 'adminmenu/return_index';
          this.enable_listing_url = 'adminmenu/return_index';
          this.hasChildList = false;
        }
        ...
        this.getlist('adminmenu/getlist');
    ...
    ionViewWillEnter() {
      ...
      //----- menu permission data start-----
        this.itemsSubscribe = this.commonUtils.menuPermissionObservable.subscribe(data => {
          if(data){
            let getpermissionArray;
            if(this.parms_action_id != null){
              // console.log('adminmenu child ');
              getpermissionArray = data['/'+this.router.url.split('/')[1]];
            }else{
              // console.log('adminmenu Parent  ');
              getpermissionArray = data[this.router.url];
            }
    ...
    // Click Delete Item end
      ...
      enabledDisabled( _identifire, _item, _items ){
        ...
        if(_identifire == 'single'){
          ...
          set_api = this.enable_listing_url+'_status/'+_item.id+'?status='+getStatus;
        }else{
          let checkItemIdArray = [];
          if(this.checkedList){
            this.checkedList.forEach(element => {
          ...
          set_api = this.enable_listing_url+'_actionall?action=statuschange&status='+_item+'&'+this.enable_listing_url+'_id='+ checkItemIdArray.join();
        }

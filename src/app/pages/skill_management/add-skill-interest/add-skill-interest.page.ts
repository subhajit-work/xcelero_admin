...
/* tslint:disable */ 
@Component({
  selector: 'app-add-skill-interest',
  templateUrl: './add-skill-interest.page.html',
  styleUrls: ['./add-skill-interest.page.scss'],
})

export class AddSkillInterestPage implements OnInit, OnDestroy {
  constructor(
  ...
  //--------------  getlist data fetch start -------------
    ...
    // ------ init function call start ------
      ...
      commonFunction(){
        ...
        // getlist data
        this.getlist('skill_qual_interest/getlist');

        if( this.parms_action_name == 'edit'){
          // form submit api edit
          this.form_api = 'skill_qual_interest/return_edit/'+this.parms_action_id;
        }else{
          // form submit api add
          this.form_api = 'skill_qual_interest/return_add'
        }
          ...
          (res:any) => {
            ...
            if(res.return_status > 0){
              this.model = {
                ...
                qual_degree: res.return_data.qual_degree,
              ...
              // show_on_homepage
              if(res.return_data.show_on_homepage){
                if(res.return_data.show_on_homepage == '1'){
                    this.model.image_enable = 'true';
                }else{
                    this.model.image_enable = '';
                }
              }

              console.log('Ressssssssssssssssssssss', res);
              // Degree
              if(res.return_data.degree){
                this.model.qual_degree = [];
                res.return_data.degree.forEach(element => {
      ...
      this.plt.ready().then(() => {
        ...
        this.getListSubscribe = this.commonUtils.getlistCommon(_getlistUrl).subscribe(
          resData => {
            ...
            this.degreeList = resData.degree;
      ...
      (response:any) => {
        ...
        if(response.return_status > 0){
          ...
          if(this.clickButtonTypeCheck == 'Save'){
            ...
            this.router.navigate(['/skill_qual_interest']);
    ...
    reloadPage(){
      if( this.parms_action_name == 'add'){
        ...
        this.model = {
          ...
          // qual_degree: this.allEditData.return_data.qual_degree,
        ...
        // image status
        if(this.allEditData.return_data.show_on_homepage){
          if(this.allEditData.return_data.show_on_homepage == '1'){
              this.model.image_enable = 'true';
          }else{
              this.model.image_enable = '';
          }
        ...
        // Degree
        if(this.allEditData.return_data.degree){
          this.model.qual_degree = [];
          this.allEditData.return_data.degree.forEach(element => {
            this.model.qual_degree.push(element.id);
          });
        }

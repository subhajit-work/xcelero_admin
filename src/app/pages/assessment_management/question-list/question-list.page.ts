  ...
  constructor(
    ...
    // ------ init function call start -------
      commonFunction(){
        ...
        this.headerUrlapi = 'question/header';
    
        // table list data url name
        this.listing_url = 'question/return_index';
      ...
      onDownload(_identifier, _items, _item){
        ...
        if(_identifier == 'multiple'){
          this.itemsSubscribe = this.authService.globalparamsData.subscribe(res => {
            this.export_url = this.main_url+'/question/download_csv?id='+items_id_array.join()+'&token='+res.token;
          });
          ...
          /* this.deleteListSubscribe = this.http.get('question/download_csv?id='+items_id_array.join()).subscribe(
            (res:any) => {
              if(res.return_status > 0){
                this.checkedList = [];
                // this.commonUtils.presentToast('success', res.return_message);

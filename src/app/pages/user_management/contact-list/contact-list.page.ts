  ...
  constructor(
    ...
    // ------ init function call start -------
      commonFunction(){
        ...
        this.headerUrlapi = 'contact/header';
    
        // table list data url name
        this.listing_url = 'contact/return_index';


        this.onHeaderData(this.headerUrlapi);
        
        // getlist data url name
        // this.getlist('contact/getlist');
      ...
      onDownload(_identifier, _items, _item){
        ...
        if(_identifier == 'multiple'){
          this.itemsSubscribe = this.authService.globalparamsData.subscribe(res => {
            this.export_url = this.main_url+'/contact/download_csv?id='+items_id_array.join()+'&token='+res.token;
          });
